    ...
    const addMilestone = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to create milestone: ${errorData.error || response.statusText}`)
            }
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {project.services?.map((service) => (
                                    <tr key={service.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-4 py-2 text-sm text-gray-900">${Number(service.price || 0).toFixed(2)}</td>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {project.milestones?.map((milestone) => (
                                    <tr key={milestone.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-2 text-sm font-medium text-gray-900">{milestone.name}</td>
                                        ...
                                        <td className="px-4 py-2 text-center">
                                            <button
                                                onClick={() => setDeleteConfirm({ type: 'milestone', id: milestone.id, name: milestone.name })}
                                                className="inline-flex items-center justify-center w-8 h-8 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-full transition-colors"
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {project.tasks?.map((task) => (
                                    <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-2 text-sm font-medium text-gray-900">{task.name}</td>
                                        ...
                                        <td className="px-4 py-2 text-center">
                                            <button
                                                onClick={() => setDeleteConfirm({ type: 'task', id: task.id, name: task.name })}
                                                className="inline-flex items-center justify-center w-8 h-8 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-full transition-colors"
                                                title="Delete task"
                                            >
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
